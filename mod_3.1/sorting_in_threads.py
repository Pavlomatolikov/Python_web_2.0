    ...
    def move(file):
        try:
            ...
            destination = Path(os.path.join(
                target_folder, file.stem + str(random.randint(1, 100)) + file.suffix))
...
def normalize(files_list):
    ...
    for i in files_list['rename']:
        if not i.is_dir():
            rename_file.append(i)
    for i in files_list['rename']:
        if i.is_dir():
            rename_dir.append(i)

    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        executor.map(rename, rename_file)
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        executor.map(rename, rename_dir)
